...
def refresh_gitlab_token(user, db: Session) -> str:
    ...
    if not user.gitlab_refresh_token:
        raise ValueError("No refresh token available. User needs to re-authenticate with GitLab.")
    ...
    if response.status_code != 200:
        raise ValueError("Failed to refresh GitLab token. User needs to re-authenticate.")
    ...
    if not new_access_token:
        raise ValueError("No access token in refresh response. User needs to re-authenticate.")
...
def get_valid_gitlab_token(user, db: Session) -> str:
    ...
    if not user.gitlab_access_token:
        raise ValueError("User has no GitLab access token. Please connect GitLab account.")
